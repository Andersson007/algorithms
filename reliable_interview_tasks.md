...
### Core tasks from three independent lists, extended with personally selected problems

> The base of 17 tasks was identified by cross-referencing three independently curated lists
> (my own, ChatGPT, and Gemini). Six additional high-value tasks were added from a personal
> selection to cover techniques not represented in the intersection. Together these 23 tasks
> form a well-rounded preparation plan for any software developer job interview.
...
1. **Two Sum II** — Find two numbers in a sorted array that add up to a target.
2. **3Sum** — Find all unique triplets in an array that sum to zero.
...
## 7. Sorting & Intervals
*Sort input or merge overlapping ranges as a key step toward the solution.*
...
17. **Permutations** — Return all possible orderings of a set of distinct numbers.
18. **Subsets** — Return all possible subsets (power set) of a set of distinct numbers.
19. **Combination Sum** — Find all unique combinations of numbers that sum to a target.
...
20. **Climbing Stairs** — Find the number of distinct ways to reach the top of N stairs.
21. **Coin Change** — Find the minimum number of coins needed to make up a given amount.
22. **Number of Ways to Make Change** — Count how many distinct combinations of coins sum to a given amount.
23. **Longest Common Subsequence** — Find the longest subsequence shared by two strings.
...
| Sorting & Intervals    | 1     | Personal pick                   |
...
| Task                     | Reason skipped                                          |
|--------------------------|---------------------------------------------------------|
| Tandem Bicycle           | Niche (AlgoExpert-specific), rarely seen in interviews  |
...
| Minimum Waiting Time     | Niche greedy; uncommon outside AlgoExpert               |
...
| Smallest Difference      | Covered implicitly by Two-Pointer technique problems    |
